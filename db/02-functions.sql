...
CREATE OR REPLACE FUNCTION get_gzels()
RETURNS TABLE (
    gzel_name VARCHAR,
    gzel_price DECIMAL(10, 2),
    gzel_species VARCHAR,
    gzel_breed VARCHAR,
    gzel_age INT,
    gzel_gender VARCHAR
) AS $$
...
CREATE OR REPLACE PROCEDURE update_customer_data(
    ...
    p_floor VARCHAR(20) DEFAULT NULL
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- Обновляем таблицу Customer
    UPDATE Customer
    SET
    ...
    -- Обновляем таблицу Customers_Addresses
...
DECLARE
    var_category_id INT;
BEGIN
    -- Если указан category_name, обновляем или добавляем запись в таблицу Category
    IF p_category_name IS NOT NULL THEN
        ...
        IF var_category_id IS NULL THEN
            ...
            RETURNING id INTO var_category_id;  -- Используем var_category_id
        ELSE
            -- Если категория найдена, обновляем её описание
            UPDATE Category
    ...
    SET
        ...
        category_id = COALESCE(var_category_id, Product.category_id),  -- Здесь указываем, что category_id обновляется для Product
